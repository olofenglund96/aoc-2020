...
space_size = np.array([xdim, ydim, 15])
...
def get_next_point(field, index, iter):
  ...
  #print(x, y, z)
  subspace_idx = np.ogrid[x-1:x+2, y-1:y+2, z-1:z+2]
  #print(subspace_idx)
  #input()
...
def get_next_field(old_field, iter):
  x, y, z = old_field.shape
  new_field = np.zeros(old_field.shape)

  for i in range(1, x-1):
    for j in range(1, y-1):
      for k in range(1, z-1):
        new_field[i, j, k] = get_next_point(old_field, (i,j,k), iter)
